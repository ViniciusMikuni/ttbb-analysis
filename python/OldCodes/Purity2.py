...
f = rt.TFile.Open("Datasets/Kinematic_Results_2011_FH_Unconstrained_Ttbar.root","READ")
t = f.Get("tkin")
...
for bin, cut in enumerate(probcut_):
    ...
    for e,event in enumerate(t) :
        if t.not_Converged or t.n_bjets != 4  : continue
        #or t.n_bjets != 4
    ...
    hexp.SetBinContent(bin+1,lumi*sigma*float(ntotal)/ngenevts)
    hexp.SetBinError(bin+1,lumi*sigma*sqrt(ntotal)/ngenevts)
