...
#            print "processing event number: ", e
            ...
            if t.nBCSVM >=2 and t.njets >=7:
                ...
                if not tth: continue
                mytree.variables['weight'][0] = t.puWeight * t.btagWeightCSV * np.sign(t.genWeight)*np.exp(0.5*(t.ngenTopLep*0.0843616-0.000743051*np.sum(t.genTopLep_pt)+t.ngenTopHad*0.0843616-0.000743051*np.sum(t.genTopHad_pt)))*(t.jets_pt[0]/t.jets_pt[0])
                corrb_ = array( 'i', 2*[ -10 ] )
                ...
                if mytree.variables['hasCorrect'][0] == 0:continue
...
#                  print t.njets, ' njets ',nbjets, ' nbjets ', ' bpos ', bpos_
                ...
                for icomb, bestcomb_ in enumerate(comb_) :
                    ...
                    if bestcomb_ in corrcomb_:
                        ...
                        if bestcomb_ == corrcomb_[0] and list(corrcomb_[0]) != corr_ :print 'sdfsdgfhshahrh', list(corrcomb_[0]), corr_
